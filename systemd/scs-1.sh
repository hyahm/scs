...
gobin=$(which go)
if [[ $? -ne 0 ]]; then
    sudo wget -O /opt/go1.15.2.linux-amd64.tar.gz https://studygolang.com/dl/golang/go1.15.2.linux-amd64.tar.gz
    cd /opt/
...
$gobin build -o scs cmd/scs/main.go
...
sudo /bin/bash -c 'cat > /etc/scs.yaml << EOF
# 监听端口
...
ExecStart=$workdir/scs/scs -f /etc/scs.yaml
