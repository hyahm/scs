...
func (node *Node) NewSCSserver() *scs.SCSserver {
	return &server.SCSserver.{
		Domain: node.Url,
		Token:  node.Token,
	}
}
...
func (node *Node) Reload() {
	...
	b, err := node.NewSCSserver.().Reload()
...
func (node *Node) Search(args string) {
	...
	for _, url := range resp.Url {
		node.NewSCSserver.().Domain = url
		b, err := node.NewSCSserver.().Search(resp.Derivative, args)
...
func (node *Node) Status(args ...string) error {
	...
	if resp.Code == 203 {
		fmt.Printf("node: %s, url: %s %v \n", node.Name, node.Url, server..ErrToken)
...
func (node *Node) Install(scripts []*script.Script, env map[string]string) {
	// 先读取配置文件
	if node.Wg != nil {
		defer node.Wg.Done()
	}
