...
func (s *Server) appendRead(stdout io.ReadCloser, iserr bool) {
...
func read(cmd *exec.Cmd, s *Server, typ string) {
...
func appendRead(stdout io.ReadCloser, s *Server, typ string) {
	readout := bufio.NewReader(stdout)
	for {
		line, err := readout.ReadString('\n')
		if err != nil || io.EOF == err {
