...
func (svc *Server) shell(command string, typ string) error {
	var cmd *exec.Cmd
	golog.Info(command)
	golog.Info(typ)
	...
	command = t + " -- " + command
	svc.LogLocker.Lock()
	svc.Log[typ] = append(svc.Log[typ], command)
...
func (s *Script) EnableScript() error {
	...
	for name := range ss.Infos[s.Name] {
		ss.Infos[s.Name][name].Disable = false
...
func (s *Script) DisableScript() error {
	...
	s.Disable = true
	for name := range ss.Infos[s.Name] {
		ss.Infos[s.Name][name].Disable = true
		go ss.Infos[s.Name][name].Stop()
	}
	return nil
}
...
func (svc *Server) wait() error {
	...
	if err := svc.cmd.Wait(); err != nil {
		...
		if svc.DeleteWhenExit {
			return Cfg.DelScript(svc.Name)
		}
