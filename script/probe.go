...
func RunProbe(p *Probe) {
	if healthDetector == nil {
		...
		if !CompareSlice(p.Monitor, healthDetector.Probe.Monitor) ||
			...
			!CompareSlice(p.ExcludeDisk, healthDetector.Probe.ExcludeDisk) {
			// 那么需要停掉之前的goroutine 然后重新启动
			healthDetector.Cancel()
			healthDetector.Probe = p
		}
	}
	global.ContinuityInterval = Cfg.Probe.ContinuityInterval
