...
func RunProbe(p *Probe) {
	if healthDetector == nil {
		...
		if !utils.CompareSlice(p.Monitor, healthDetector.Probe.Monitor) ||
			...
			!utils.CompareSlice(p.ExcludeDisk, healthDetector.Probe.ExcludeDisk) {
			// 那么需要停掉之前的goroutine 然后重新启动
			healthDetector.Cancel()
			healthDetector.Probe = p
		}
	}
	global.ContinuityInterval = p.ContinuityInterval
	if global.ContinuityInterval == 0 {
		global.ContinuityInterval = time.Hour * 1
	}
